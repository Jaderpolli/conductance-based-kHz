...
bifurcation_file = "./11_Cortex_Erisir.dat"
ISI_file = "../../../../results/raw/mdl_properties/ISI_Erisir_2.txt"
...
def read_xppdat(filename_bifurcation, filename_ISI):
    ...
    unstable_Imin = 6 # np.min(unstable_Istim)
    unstable_Imax = 117
    stable_C1max = 10
    stable_C2min = 110
    ...
    ax[0].set_ylim([ISI_min, 47])
    ax[0].plot(all_I0, ISI, color=color)
    ax[0].tick_params(axis='y', labelcolor=color)
    ax[0].grid()
    fig.suptitle("Cortical neuron model")
    ...
    fig.tight_layout()
    fig.savefig('11_Cortex_Erisir.pdf')
    plt.show()
